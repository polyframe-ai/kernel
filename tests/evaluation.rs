...
fn test_dataset_discovery() {
    ...
    assert!(
        models.len() >= 6,
        "Expected at least 6 example models, found {}",
        models.len()
    );
...
fn test_report_generation() {
    ...
    evaluation::Reporter::write_json(&report, &json_path).expect("Failed to write JSON");
    evaluation::Reporter::write_markdown(&report, &md_path).expect("Failed to write Markdown");
