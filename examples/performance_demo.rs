...
fn main() {
    ...
    let child2 = Node::with_id(
        NodeKind::Sphere { r: 7.0, fn_: 64 },
        "sphere1".into()
    );
    
    let child3 = Node::with_id(
        NodeKind::Cylinder { h: 20.0, r: 4.0, fn_: 64 },
        "cylinder1".into()
    );
    ...
    println!("   ✓ Cache: {}/{} nodes ({:.1}% hit rate)\n", 
    ...
    println!("   🚀 Speedup: {:.2}×\n", 
        report.speedup("full_evaluation", "incremental_cached").unwrap_or(1.0));
    ...
    let updated_sphere = Node::with_id(
        NodeKind::Sphere { r: 10.0, fn_: 64 },
        "sphere1".into()
    );
    
    let timer = Timer::start();
    let mesh = kernel.update_subtree(&"sphere1".to_string(), updated_sphere).unwrap();
