...
impl Triangle {
    ...
    /// Calculate face normal from triangle geometry using cross product
    /// This computes the normal from the triangle's actual geometry, not vertex normals
    /// Returns a normalized vector pointing outward (right-hand rule)
    pub fn face_normal(&self, mesh: &Mesh) -> Vector3<f32> {
        let v0 = &mesh.vertices[self.indices[0]];
        let v1 = &mesh.vertices[self.indices[1]];
        let v2 = &mesh.vertices[self.indices[2]];
