...
fn export_stl_binary(mesh: &Mesh, path: &Path) -> Result<()> {
    ...
    let triangles: Vec<StlTriangle> = mesh.triangles.iter().map(|tri| {
...
fn export_stl_ascii(mesh: &Mesh, path: &Path) -> Result<()> {
    ...
    for tri in &mesh.triangles {
        ...
        writeln!(file, "      vertex {} {} {}", v0.position.x, v0.position.y, v0.position.z)?;
        writeln!(file, "      vertex {} {} {}", v1.position.x, v1.position.y, v1.position.z)?;
        writeln!(file, "      vertex {} {} {}", v2.position.x, v2.position.y, v2.position.z)?;
