...
use polyframe::{render, geometry::Primitive, ast::*, Kernel};
...
fn bench_incremental_vs_full(c: &mut Criterion) {
    ...
    let child3 = Node::with_id(
        NodeKind::Cylinder { h: 20.0, r: 3.0, fn_: 32 },
        "cylinder1".into()
    );
    ...
    group.bench_function("kernel_update_subtree", |b| {
        ...
        let updated_child = Node::with_id(
            NodeKind::Sphere { r: 8.0, fn_: 32 },
            "sphere1".into()
        );
        
        b.iter(|| {
            kernel.update_subtree(
...
fn bench_parallel_vs_sequential(c: &mut Criterion) {
    ...
    group.bench_function("parallel", |b| {
        b.iter(|| {
            ParallelEvaluator::evaluate(black_box(&union_node)).unwrap()
        });
    });
...
fn bench_memory_usage(c: &mut Criterion) {
    ...
    let large_union: Vec<Node> = (0..50)
        .map(|i| {
            Node::new(NodeKind::Cube(Vector3::new(
                5.0 + i as f32 * 0.1,
                5.0,
                5.0
            )))
